...
def main():
    ...
    node_rank = config["rank"]
    config["rank"] = mpi_rank
    device_local_no = mpi_rank % config["nwpernode"]
    ...
    device = torch.device('cuda:{}'.format(device_local_no))
...
def get_reducer(device, timer):
    ...
    if (
        ...
        or config["optimizer_reducer"] == "ExDynaReducer"
