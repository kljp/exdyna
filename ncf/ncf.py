...
def parse_args():
    ...
    parser.add_argument('--nwpernode', type=int, required=True)
...
def init_distributed(args, device_local_no):
    args.distributed = args.world_size >= 1

    if args.distributed:

        torch.cuda.set_device(device_local_no)
        args.local_rank = device_local_no        
...
def main():
    ...
    args.rank = mpi_rank
    device_local_no = mpi_rank % args.nwpernode

    init_distributed(args, device_local_no)
    ...
    if args.reducer=='exact':
        ...
        reducer = getattr(gradient_reducers, "ExDynaReducer")(1, device, timer, compression=args.comp_ratio)
        run_name = args.reducer+'_'+str(args.comp_ratio)+'_'+str(args.seed)
